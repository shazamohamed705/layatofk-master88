...
function ShareAdds() {
  ...
  // Get country_id from URL params or default to 1
  const countryId = useMemo(() => searchParams.get('country_id') || '1', [searchParams])

  // Fetch categories from API using GET method
  const fetchCategories = useCallback(async () => {
    try {
      setLoading(true)
      setError(null)
      
      const endpoint = `/api/adv_home?country_id=${countryId}`
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {sortedCategories.map((category) => {
                const categoryUrl = `/share-car?category_id=${category.id}&category_name=${encodeURIComponent(category.name)}`
                ...
                return (
                  <Link
                    key={category.id}
                    to={categoryUrl}
                    className="group relative bg-white rounded-lg shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-100 hover:border-primary/30"
                  >
                    {/* Card Content */}
